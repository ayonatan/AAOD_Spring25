    ...
    def update_H_no_pruning(self, F, H, group_id , tau=0 , violation_vector='CVV'):
        ...
        for agg_value in agg_values:
            if F[agg_value] <= 0: # no remaining tuples for this agg value
                continue
            while H_index < len(H_keys) and H_keys[H_index] <= agg_value+tau: # find the largest agg value in H that is smaller than or equal to agg_value
                # H : all possible solutions we have so far
                # H_keys: sorted keys of H
                # H_keys[H_index] <= agg_value +tau: we are looking for the largest agg value in H that is smaller than or equal to agg_value
